...
I put the whole setup in a small, standalone repo you can clone, fork, or star: [**github.com/matanhakim/local-voice-llm**](https://github.com/matanhakim/local-voice-llm). It has three parts: installing the LLM agent (Claude Code), the push-to-talk dictation daemon described above, and the transcription script and skill, along with a recipe you can hand to Claude Code or any coding agent to set local transcription up from scratch on your own machine. The recipe tells the agent to inspect your hardware, pick a model that fits your RAM and your language, install faster-whisper, and drop in a transcribe script you can reuse. The speech always runs locally: no cloud, no API key, no meter.
